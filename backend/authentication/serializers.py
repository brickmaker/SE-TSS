...
class StudentSerializer(PeopleSerializer):
    ...
    def create(self, validated_data):
        ...
        #Log.objects.create(content="[user]: %s [event]: create student" % self._context['request'].user.username)
        ...
        instance = Account.objects.create_user(id_number=id_number, user_type=user_type, username=username, email=email,
                                               gender=gender, name=name, department=department, grade=grade,
                                               major=major, class_name=class_name)
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Course
        fields = (
        'course_id', 'name', 'credit', 'semester', 'capacity', 'assessment', 'faculty', 'course_type', 'state',
        'department')

    def create(self, validated_data):
        logger.info("create course")
        #Log.objects.create(content="[user]: %s [event]: create course" % self._context['request'].user.username)
        ...
        instance = Course.objects.create(semester=semester, department=depart, course_id=course_id, name=name,
                                         capacity=capacity, credit=credit, assessment=assessment,
                                         course_type=course_type, state=state)
    ...
    def update(self, instance, validated_data):
        ...
        Log.objects.create(content="[user]: %s [event]: update course info" % self._context['request'].user.username)
        try:
            depart = Department.objects.get(name=validated_data.get('department'))
        except Exception as err:
            raise ParseError(detail='department not exists')
...
class AccountSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        Log.objects.create(content="[user]: %s [event]: update account password" % self._context['request'].user.username)
...
class PasswordUpdateSerializer(serializers.Serializer):
    ...
    def validate_new_password(self, value):
        logger.info("update account password")
        #Log.objects.create(content="[user]: %s [event]: update password" % self._context['request'].user.username)
...
class StudentQuerySerializer(PeopleSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        Log.objects.create(content="[user]: %s [event]: update student info" % self._context['request'].user.username)
        # student: email, img
...
class FacultyQuerySerializer(PeopleSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        Log.objects.create(content="[user]: %s [event]: update faculty info" % self._context['request'].user.username)
        # faculty: email, img
...
class StaffQuerySerializer(PeopleSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        Log.objects.create(content="[user]: %s [event]: update staff info" % self._context['request'].user.username)
...
class AdminQuerySerializer(PeopleSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        Log.objects.create(content="[user]: %s [event]: update admin info" % self._context['request'].user.username)
...
class CourseQuerySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Course
        fields = (
        'course_id', 'name', 'credit', 'capacity', 'semester', 'department', 'assessment', 'state', 'course_type')
...
class DepartmentQuerySerializer(serializers.ModelSerializer):
    ...
    major_for = serializers.StringRelatedField(many=True)

    class Meta:
        model = Department
        fields = ('name', 'major_for')
...
class MajorQuerySerializer(serializers.ModelSerializer):
    ...
    class_name_for = serializers.StringRelatedField(many=True)

    class Meta:
        model = Major
        fields = ('depart', 'major', "class_name_for")
        # read_only_fields =('major',"class_name")

class LogQuerySerializer(serializers.ModelSerializer):
    class Meta:
        model = Log
        fields = ('time', 'content')
